...
void CNewWizDialog::SetFocusAndDef(CNewWizPage *pPage)
{
	// Get the first control which has "Tab Stop" property
	// and not disabled
	CWnd* pFocusWnd=pPage->GetNextDlgTabItem((CWnd *)NULL,false);
	while (pFocusWnd && !pFocusWnd->IsWindowEnabled())
		pFocusWnd=pPage->GetNextDlgTabItem(pFocusWnd,false);
	if (!(pFocusWnd->GetStyle() & WS_TABSTOP)) 
		pFocusWnd=NULL;
	if (pPage==GetLastPage())
	{
		SetDefID(ID_WIZFINISH);
	    if (pFocusWnd)
			pFocusWnd->SetFocus();
		else
	...
	else
	{
		...
		if (pFocusWnd)
			...
			GetDlgItem(ID_WIZNEXT)->SetFocus();
	}
